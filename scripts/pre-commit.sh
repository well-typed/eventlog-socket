...
# Check for unstaged changes in C files:
UNSTAGED_C_FILES="$(git ls-files --exclude-standard --no-deleted --deduplicate --modified '*.c' '*.h')"
if [ ! "${UNSTAGED_C_FILES}" = "" ]; then
  echo "Found unstaged C files"
  echo "${UNSTAGED_C_FILES}"
  echo
  echo >"${FAIL}"
fi
...
echo
(./scripts/format-clang-format.sh || echo >"${FAIL}")
echo
